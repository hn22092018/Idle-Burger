...
namespace SDK {
    ...
    public class AdsManager : MonoBehaviour {
        ...
        private double m_InterstitialFirstInstallCap = 0;
        ...
        public bool IsActiveBannerAds = false;
...
#if UNITY_EDITOR
            IsActiveBannerAds = true;
#endif
            SetFirstInstallCap(ProfileManager.PlayerData.timeManager.DayRealPlayed == 1 ? 600 : 0);
        }
        private void Start() {
        ...
        private void UpdateRemoteConfigs() {
            {
                ...
                Debug.Log("=============== InterTimeCapping " + m_MaxInterstitialCappingAdsTime);
                ResetAdsInterstitialCappingTime();
            }
            {
                ...
                Debug.Log("=============== ActiveInterruptAds " + IsActiveInterruptReward);
            }
            {
                ...
                Debug.Log("=============== InterruptAdsTime " + m_MaxRewardInterruptCount);
            }
            {
                ConfigValue configValue = ABIFirebaseManager.Instance.GetConfigValue(ABI.Keys.key_remote_active_banner_ads);
                IsActiveBannerAds = configValue.BooleanValue;
#if UNITY_EDITOR
                IsActiveBannerAds = true;
#endif
                if (IsGoodToShowBannerAds()) {
                    SetupBannerAds();
                }
                Debug.Log("=============== ActiveBannerAds " + IsActiveBannerAds);
            }
        }
        private void Update() {
            float dt = Time.deltaTime;
            if (m_InterstitialFirstInstallCap > 0) {
                m_InterstitialFirstInstallCap -= dt;
            }
        ...
        public void ShowInterstitial(UnityAction closedCallback, UnityAction showSuccessCallback, bool isTracking = true, bool isSkipCapping = false) {
            if (m_InterstitialFirstInstallCap > 0) return;
            if (!isSkipCapping) {
        ...
        private void OnInterstitialAdShowSuccess() {
            ...
            EventManager.TriggerEvent("DeactiveAOA");
        ...
        public void SetFirstInstallCap(double time) {
            m_InterstitialFirstInstallCap = time;
        }
        #endregion
        ...
        public void SetupBannerAds() {
            ...
            for (int i = 0; i < m_BannerAdsConfig.adsMediations.Count; i++) {
                m_BannerAdsConfig.adsMediations[i].InitBannerAds();
            }
        }
        ...
        public bool IsGoodToShowBannerAds() {
            return IsActiveBannerAds && !ProfileManager.PlayerData.ResourceSave.activeRemoveAds;
        }
        #endregion
        ...
        public void ShowRewardVideo(string rewardedPlacement, UnityAction successCallback, UnityAction failedCallback = null) {
            ...
            if (IsActiveInterruptReward && IsReadyToShowRewardInterrupt() && IsInterstitialAdLoaded() && m_InterstitialFirstInstallCap <= 0) {
